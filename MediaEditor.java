...
public class MediaEditor extends JFrame {

	private JPanel panel, panel1, panel2, panel3, panel4, panel5;
	private JButton openfile, stop, forward, backward, mute, edit;
	...
	public MediaEditor() {
		...
		panel4.add(time,BorderLayout.WEST);
		move = new JSlider();
		move.setValue(0);
		move.setPreferredSize(new Dimension(this.getWidth() - 370, 20));
		panel4.add(move);
		...
		editor.setPreferredSize(new Dimension(230, panel.getHeight()));
		...
		ticker = new Timer(1000, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if((int)video.getLength()==move.getValue()){
					if (speedup != null) {
						speedup.cancel(true);
						speedup = null;
						video.play();
		...
		// public void actionPerformed(ActionEvent e) {
		// String cmd =
		// "avconv -i input.avi -vf drawtext=fontcolor=white:fontsize=30:fontfile=/usr/share/fonts/truetype/ubuntu-font-family/Ubuntu-L.ttf:text='$date':x=30:y=h-text_h-30 ";
		// cmd = cmd + mp3;
		//
